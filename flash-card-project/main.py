...
def next_card():
    ...
    canvas.itemconfig(card_background, image=flash_front_img)
    flip_timer = window.after(3000, func=flip_card)


def is_known():
    to_learn.remove(current_card)
    data_csv = DataFrame(to_learn)
...
def flip_card():
    ...
    canvas.itemconfig(card_background, image=flash_back_img)
...
card_background = canvas.create_image(400, 263, image=flash_front_img)
